...
public class RestClient implements Closeable {
    ...
    public synchronized void setHosts(HttpHost... hosts) {
        ...
        this.hostTuple = new HostTuple<Set<HttpHost>>(Collections.unmodifiableSet(httpHosts), authCache);
    ...
    public void performRequestAsync(String method, String endpoint, Map<String, String> params,
        ...
        try {
            ...
            if (ignoreString == null) {
                ...
                ignoreErrorCodes = new HashSet<Integer>();
                if (HttpHead.METHOD_NAME.equals(method)) {
                    //404 never causes error if returned for a HEAD request
    ...
    private HostTuple<Iterator<HttpHost>> nextHost() {
        ...
        do {
            Set<HttpHost> filteredHosts = new HashSet<HttpHost>(hostTuple.hosts);
            ...
            if (filteredHosts.isEmpty()) {
                ...
                List<Map.Entry<HttpHost, DeadHostState>> sortedHosts = new ArrayList<Map.Entry<HttpHost, DeadHostState>>(blacklist.entrySet());
                if (sortedHosts.size() > 0) {
                    Collections.sort(sortedHosts, new Comparator<Map.Entry<HttpHost, DeadHostState>>() {
                        ...
                        public int compare(Map.Entry<HttpHost, DeadHostState> o1, Map.Entry<HttpHost, DeadHostState> o2) {
                            long x = o1.getValue().getDeadUntilNanos();
                            long y = o2.getValue().getDeadUntilNanos();
                ...
                List<HttpHost> rotatedHosts = new ArrayList<HttpHost>(filteredHosts);
    ...
    private static HttpRequestBase createHttpRequest(String method, URI uri, HttpEntity entity) {
        String flag = method.toUpperCase(Locale.ROOT);
        if(HttpDeleteWithEntity.METHOD_NAME.equals(flag)){
            return addRequestBody(new HttpDeleteWithEntity(uri), entity);
        }else if(HttpGetWithEntity.METHOD_NAME.equals(flag)){
            return addRequestBody(new HttpGetWithEntity(uri), entity);
        }else if( HttpHead.METHOD_NAME.equals(flag)){
            return addRequestBody(new HttpHead(uri), entity);
        }else if( HttpOptions.METHOD_NAME.equals(flag)){
            return addRequestBody(new HttpOptions(uri), entity);
        }else if( HttpPatch.METHOD_NAME.equals(flag)){
            return addRequestBody(new HttpPatch(uri), entity);
        }else if( HttpPost.METHOD_NAME.equals(flag)){
            HttpPost httpPost = new HttpPost(uri);
            addRequestBody(httpPost, entity);
            return httpPost;
        }else if( HttpPut.METHOD_NAME.equals(flag)){
            return addRequestBody(new HttpPut(uri), entity);
        }else if( HttpTrace.METHOD_NAME.equals(flag)){
